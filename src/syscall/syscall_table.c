...
static long emul_syscall(pid_t pid, uint64_t args[6]) {
    ...
    BSD_TRACE("indirect syscall: real syscall = %d", real_syscall);
    
    /* Look up and execute the real syscall */
    ...
    /* For translated syscalls, execute directly */
    ...
    /* For translated syscalls, return the Linux number for interceptor to handle */
    return -ENOSYS;  /* Let interceptor handle it normally */
}
