 ...
 * 
 * This creates the directory structure for the container but doesn't mount yet.
 * The mount happens at container start time.
 */
...
/*
 * Mount the container's overlay filesystem
 * 
 * This combines the base image (lowerdir) with the container's changes (upperdir)
 * into a unified view (merged).
 */
int lochs_storage_mount_container(lochs_jail_t *jail) {
     ...
     *   workdir  = overlay work directory (required by overlayfs)
...
/*
 * Unmount the container's overlay filesystem
 */
int lochs_storage_unmount_container(lochs_jail_t *jail) {
    ...
    /* Even if umount fails (e.g., not mounted), clear the flag */
    jail->overlay_mounted = 0;
 ...
 * 
 * This removes the diff, work, and merged directories.
